...
def main():
    ...
    # ─────────────────────────────────────────────────────────────────────
    # STEP 4: ACCESSIBILITY (axe-core) — reduced pages in fast mode
    # ─────────────────────────────────────────────────────────────────────
    step_start = time.time()
    print(f"{'#'*60}")
    ...
    try:
        ...
        a11y_page_limit = 1 if scan_mode == "fast" else 3
        pages_to_check = pages[:a11y_page_limit]
        print(f"[A11y] Running axe-core on {len(pages_to_check)} pages (fast_mode={scan_mode == 'fast'})...")
    ...
    # ─────────────────────────────────────────────────────────────────────
    # STEP 5: MOBILE RESPONSIVENESS — reduced pages + breakpoints in fast mode
    # ─────────────────────────────────────────────────────────────────────
    step_start = time.time()
    print(f"{'#'*60}")
    ...
    try:
        ...
        pages = db.get_pages(scan_id)
        mobile_page_limit = 1 if scan_mode == "fast" else 3
        pages_to_check = [p for p in pages if p.get("status_code") and 200 <= p["status_code"] < 400][:mobile_page_limit]
        print(f"[Mobile] Testing {len(pages_to_check)} pages (fast_mode={scan_mode == 'fast'})...")
        raw_mobile = asyncio.run(run_mobile_checks(pages_to_check, max_pages=mobile_page_limit, fast_mode=(scan_mode == "fast")))
...
if __name__ == "__main__":
    ...
    # Steps 4-6: Parallel lightweight checks (skip in fast mode)
    step_start = time.time()
    print(f"{'#'*60}")
    print(f"  STEPS 4-6: TECH/FEATURES/CTA/SECURITY (PARALLEL)")
    print(f"{'#'*60}")
    ...
    step_elapsed = time.time() - step_start
    print(f"  >> Steps 4-6 done in {step_elapsed:.1f}s\n")

    # Step 7: Compute overall score
    from analyzers.scoring import compute_overall_score
    overall_score = compute_overall_score(
        overall["ui"]["score"] if "ui" in overall else 0,
        overall["ux"]["score"] if "ux" in overall else 0,
        overall["seo"]["score"] if "seo" in overall else 0,
        mobile_score=mobile_score,
        missing_features_score=missing_features_score,
        cta_score=cta_score,
        security_score=security_score,
    )
    db.update_scan(scan_id, overall_score=overall_score["overall_score"], mobile_score=mobile_score, missing_features_score=missing_features_score, cta_score=cta_score, security_score=security_score)
    ...
    if scan_mode == "fast":
        ...
        print(f"  UI Score:        {overall['ui']['score']}/100 ({overall['ui']['grade']})")
        print(f"  UX Score:        {overall['ux']['score']}/100 ({overall['ux']['grade']})")
        print(f"  SEO Score:       {overall['seo']['score']}/100 ({overall['seo']['grade']})")
        print(f"  Mobile Score:    {mobile_score}/100")
        print(f"  Features Score:  {missing_features_score}/100")
        print(f"  CTA Score:       {cta_score}/100")
        print(f"  Security Score:  {security_score}/100")
